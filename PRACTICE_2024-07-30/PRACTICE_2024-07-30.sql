...
-- 원인 ? 사용자 계정을 생성한 후에 권한을 부여하지 않았다.
-- 해결방안 ? 관리자 계정으로 접속한 후에 해당 사용자에 최소한의 권한(CONNECT, RESOURCE)을 부여해줘야 한다.
--          관리자 계정으로 접속한 후 CREATE SESSION 권한을 부여해줘야 함 GRANT CONNECT, RESOURCE TO C##TEST;
...
--        TB_EMP의 JOBNO와 TB_JOB의 JOBCODE 컬럼 이름이 다르다.
-- 해결방안 ? JOIN~USING 구문을 JOIN~ON구문으로 변경 
--          FROM TB_EMP
