/*
    * 트리거 (TRIGGER)
    : 지정한 테이블에 DML(INSERT, UPDATE, DELETE)문에 의해 변경사항이 있을 때 (테이블에 이벤트가 발생했을 때)
      ...
      EX) 회원 탈퇴 시 기존 회원 테이블에서 데이터 삭제 (DELETE) 하고
            탈퇴 회원 테이블에 데이터 추가(INSERT) 해야할 때 자동으로 실행
            
      EX) 신고 횟수가 특정 값을 넘어갔을 때 해당 회원을 블랙리스트 처리
      
      EX) 입출고에 대한 데이터를 기록할 때 해당 상품의 재고수량을 갱신해야할  때
      ...
      * 트리거의 종류 *
      ...
      - SQL문의 의해 영향을 받는 각 행에 따른 분류
        ...
        * 트리거 생성하기 *
        ...
        BEFORE|AFTER    INSERT|UPDATE|DELETE ON 테이블명     --0이벤트 관련 정보
...
SELECT * FROM TB_PRODUCT;
SELECT * FROM TB_PDETAIL ORDER BY DNO;
