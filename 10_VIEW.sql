...
/*
    * DML 명령어로 조작이 불가능한 경우가 많음!
    ...
    2) 뷰에 정의되어 있지 않고 베이스테이블(실제 테이블)에서 NOT NULL 제약조건이 지정되어 있는 경우
...
UPDATE VW_EMP
    SET SALARY = 2000000
 WHERE EMP_ID = 204;    --> 서브쿼리에 작성한 조건에 부합되지 않기 때문에 변경 불가 (=> 오류발생!)
...
DELETE FROM VW_EMP WHERE EMP_ID = 200;  --> READ ONLY 옵션에 의하여 DML 사용불가!
