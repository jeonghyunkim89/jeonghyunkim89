...
/*
    ...
    * 객체 접근 권한 종류 * CRUD
    ...
    --> TEST계정에 KH계정의 EMPLOYEE 테이블을 조회할 수 있는 권한을 부여    

    * 권환 회수 *
    REVOKE 회수할 권한 FROM 계정명;
    ex) REVOKE SELECT ON KH.EMPLOYEE FROM TEST;
    --> TEST계정에 부여했던 KH계정의 EMPLOYEE 테이블을 조회할 수 있는 권한을 회수
*/
/*
    ROLE(롤, 규칙) : 특정 권한들을 하나의 집합으로 모아놓은 것
    
    - CONNECT : 접속 권한(CREATE SESSION)
    - RESOUCE : 자원 관리(특정 객체 생성 권한) (CREATE TABLE, CREATE VIEW, ...)
*/
--롤 정보를 조회
...
/*
    ...
    * 트랜잭션 종류
        ...
        - ROLLBACK : 트랜직션에 담겨있는 변경사항들을 삭제(취소)하고,
...
-- DDL문 사용 전 DML로 사용된 쿼리들이 있다면 확실하게 처리(COMMIT/ROLLBACK)한 후에 DDL을 사용해야함~!
