...
SELECT EMP_ID "사원 번호", EMP_NAME 이름, SALARY AS 월급
FROM EMPLOYEE;
...
FROM EMPLOYEE
WHERE JOB_CODE = 'J7' -- (1) 직급코드가 : 'J7' 인
        AND SALARY >= 2000000; -- (2) 급여가 200만원 이상인

-- 전체 사원 정보를 최근 입사일 기준으로 정렬하여 조회
SELECT * 
FROM EMPLOYEE
ORDER BY HIRE_DATE DESC;
-- *기본값* ASC NULLS LAST -- NULL값을 뒤쪽에 배치하여 조회
--         DESC NULLS FIRST -- NULL값을 앞쪽에 배치하여 조회
---------------------------------------------