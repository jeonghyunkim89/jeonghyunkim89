      ...
      * 시퀀스 생성하기 *
      ...
      [NOCACHE | CACHE 숫자] --> 캐시메모리 할당 여부 [기본값 : CACHE 20]
                            --> 캐시메모리 : 미리 발생될 값들을 생성해서 저장해두는 공간
                                           ...
                                           캐시 메모리라는 공간에 미리 생성된 값들을 가져다가 사용 (속도가 빠름!)
    [참고]
    - 이름 지어주기 -
    * 테이블 : TB_
    * 뷰 : VW_
...
*/
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;  --오류발생! NEXTVAL를 한번도 수행하지 않은 이상 CURRVAL는 사용할 수 없다.
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; --생성 후 처음 수행시 시작값 : 300
SELECT SEQ_EMPNO.CURRVAL FROM DUAL; --300
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; --3O5
