...
/*
    *서브쿼리의 종류*
    ...
    - 다중행 다중열 : 서브쿼리의 수행 결과가 여러행 여러컬럼일 때 (N행 N열)
...
-- 노옹철 사원의 급여보다 많이 받는 사원의 사원명, 부서코드, 급여조회
...
*/
--유재식 사원 또는 윤은해 사원과 같은 직급인 사원들의 정보를 조회 (사번/사원명/직급코드/급여)
--*유재식 사원 또는 윤은해 사원의 직급코드 조회
SELECT JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME IN ('유재식', '윤은해');

--*'J3'또는 'J7' 사원들의 정보를 조회
SELECT *
FROM EMPLOYEE
...
WHERE E.JOB_CODE = J.JOB_CODE
    ...
    AND JOB_NAME = '대리';
/*
    다중열 서브쿼리 : 서브쿼리 수행 결과가 행은 하나이고, 컬럼(열) 수가 여러개인 경우
*/
...
WHERE (JOB_CODE, MANAGER_ID) = (SELECT JOB_CODE, MANAGER_ID
                                FROM EMPLOYEE
                                 WHERE EMP_NAME = '박나라')
    AND EMP_NAME != '박나라'; --(박나라 사원 제외하고자 한다면)
    ------------------------------------------------------------
    /*
        다중행 다중열 서브쿼리 : 서브쿼리의 결과가 여러행, 여러개의 열인 경우
    */
    --각 직급별 최소급여를 받는 사원정보를 조회
    --1) 각 직급별 최소급여
...
--*보너스 포함 연봉이 NULL이 아니어야 하고, 보너스 포함 연봉이 3000만원 이상인 사람들만 조회
SELECT EMP_ID 사번, EMP_NAME 이름,(SALARY+(SALARY*NVL(BONUS,0)))*12 보너스포함연봉, DEPT_CODE 부서코드
FROM EMPLOYEE
WHERE (SALARY+(SALARY*NVL(BONUS,0)))*12 >= 30000000
ORDER BY 3 DESC;
--> TOP-N분석 : 상위 N개를 조회
SELECT ROWNUM, 사번, 이름, 보너스포함연봉, 부서코드
FROM (SELECT EMP_ID 사번, EMP_NAME 이름,(SALARY+(SALARY*NVL(BONUS,0)))*12 보너스포함연봉, DEPT_CODE 부서코드
        FROM EMPLOYEE
        WHERE (SALARY+(SALARY*NVL(BONUS,0)))*12 >= 30000000
        ORDER BY 3 DESC )
...
/*
    ...
    -RANK()OVER(정렬기준) : 동일한 순위 이후의 등수를 동일한 수 만큼 건너뛰고 순위계산
    -DENSE_RANK()OVER(정렬기준) : 동일한 순위가 있더라도 그 다음 등수를 +1해서 순위계산
    SELECT절에서만 사용 가능!
*/
...
FROM(SELECT EMP_NAME, SALARY, DENSE_RANK()OVER(ORDER BY SALARY DESC) "순위"
    FROM EMPLOYEE)
WHERE ROWNUM <= 5; --> 오류발생! WHERE절에서는 별칭 사용 불가!
...
--문제점 (원인): TOP-N분석 방식의 ROWNUM함수 활용하고 또 정렬하는 ORDER BY를 다시 사용하였다.
--> 선생님 답안 : 정렬이 되기전에 5명이 추려졌음
--해결방안 : ORDER BY절 삭제, FROM EMPLOYEE절에 (SELECT EMP_NAME 이름, SALARY 급여
                                -- FROM EMPLOYEE
                                -- ORDER BY SALARY DESC)
                        -- WHERE ROWNUM <= 5;
...
--문제점(원인): 부서별 평균급여가 270만원을 초과하는 부서만 해당되지 않는다. 
--해결방안: SELECT DEPT_CODE, SUM(SALARY)"총합", FLOOR(AVG(SALARY)) AS "평균", COUNT(*) "인원수"
       -- FROM EMPLOYEE
       -- GROUP BY DEPT_CODE
