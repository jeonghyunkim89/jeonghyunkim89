...
SELECT JOB_CODE, COUNT(*) "총 사원수", COUNT(BONUS) "보너스 받는 사원수"
        , SUM(SALARY) "급여 합", ROUND(AVG(SALARY)) "평균급여", MIN(SALARY) "최저급여", MAX(SALARY) "최고급여"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE --부서코드 기준으로 그룹화하고, 그 내에서 직급코드 기준으로 세부그룹화를함
                            --조건1순위 먼저 쓰고, 그 다음 조건2순위를 써준다.
ORDER BY DEPT_CODE;
-------------------------------------------------------------------------------
/*
    HAVING절
...
*/
--각 부서별 평균 급여 조회(부서코드, 평균급여->반올림처리)
...
GROUP BY DEPT_CODE
HAVING COUNT(BONUS) = 0; --그룹화한 후에 COUNT함수를 사용하여 BONUS받는 사원이 없는 조건을 제시해야함
